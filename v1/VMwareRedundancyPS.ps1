    ...
    if ($status1 -eq "PoweredOn")
    {
        ...
        else 
        {
            ...
            if (!(Test-Connection -Cn 10.156.4.97 -BufferSize 16 -Count 3 -ea 0 -Quiet))
            {
                Write-Host "$computer1 returned no response"
                Start-Sleep -Seconds 1
                Write-Host "Attempting to restart Net Adapter on $computer1"
                $job = Start-Job -ScriptBlock { Invoke-Command -Computername JJ-PSRV1 -ScriptBlock {Restart-NetAdapter -Name "Ethernet0"} }
                $job | Wait-Job -Timeout ( 15 ) | Remove-Job
